...
void init_session() {
  ...
  zp_config_insert(z_loan(config), Z_CONFIG_MODE_KEY, z_string_make(ZENOH_MODE_KEY));
  zp_config_insert(z_loan(config), Z_CONFIG_CONNECT_KEY, z_string_make(ZENOH_CONNECT_KEY));
  ...
  if (zp_start_read_task(z_loan(session), NULL) < 0 ||
      zp_start_lease_task(z_loan(session), NULL) < 0) {
